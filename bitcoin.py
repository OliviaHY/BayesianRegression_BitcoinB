...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
      similarity = 0
      yi = Xi.iloc[i][-1]
      Xistd = np.std(Xi.iloc[i][:-1])
      Ximean = np.mean(Xi.iloc[i][:-1])
      similarity = sum((X[:-1] - Xmean)*(Xi.iloc[i][:-1] - Ximean))/float(M*Xstd*Xistd)
      temp = math.exp(wt*similarity)
      top = yi*temp
      button = temp
      tops.append(top)
...
def similarity(a,b):
  ...
  bmean = np.mean(b)
  bstd = np.std(b)
  similarity = sum((a-amean)*(b-bmean))/(len(a)*astd*bstd)
  return similarity
